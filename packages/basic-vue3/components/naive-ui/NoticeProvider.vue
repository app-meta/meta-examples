...
<script setup>
    ...
    import { useMessage, useNotification, useDialog,NButton, useLoadingBar, NTable, NInput, NInputNumber, NTr, NTd, NTh, NTabs, NTabPane, NText } from "naive-ui"
    ...
    const renderTable = (data, filename)=> ()=> {
        ...
        let items = [
            h(
                ...
                ()=> data.map((row, rowIndex)=> h(NTr, ()=>row.map(cell=> h(rowIndex==0?NTh:NTd, ()=>cell))))
            )
        ]
        if(!!filename) {
            items.push(
    ...
    window.M = window.M || {
         ...
         */
        showData (data, ps={}){
            ps = Object.assign({title:"数据展示", width: "800px", showIcon: true, transformOrigin:"center"}, ps)
            if(Array.isArray(data) && Array.isArray(data[0])){
                ps.content = renderTable(data, ps.filename)
            }
            else if(typeof(data) === 'string')
                ps.content = UI.html(data)
                ...
                ps.content = ()=>h(
                    ...
                    ()=>Object.keys(data).map((title, index)=>h(
                        ...
                        Array.isArray(data[title])?
                            renderTable(data[title], ps.filename)
                            :
                            UI.html(data[title])
                    ))
                )
